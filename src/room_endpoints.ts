...
import { ValidateAndExtractUserID, GetUserByID, GetRoomUserDataFromUser } from './user_service';
import { ValidateAndExtractRoomID, GetRoomByID } from './room_service';
...
export const RegisterRoomEndpoints = (app) =>
{
    ...
    app.get("/rooms/create_room", (req, res) =>
    {
        ...
        GetUserByID(user_id,
            (db_user) => // Found
            {
                ...
                let room_user_data = GetRoomUserDataFromUser(db_user);
    ...
    {
        let room_id = ValidateAndExtractRoomID(req.query.room_id, (err) =>
        {
            console.log(err);
            res.sendStatus(400);
            return;
